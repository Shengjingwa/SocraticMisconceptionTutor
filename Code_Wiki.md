...
近期经过重构后，项目的模型调用已全面迁移至阿里云百炼 API（`qwen-plus` 用于教学与模拟学生，`deepseek-chat` 用于打分），并启用了深度思考（`enable_thinking`）能力以增强推理。同时，状态管理升级为不可变的 Pydantic BaseModel，确保了架构的幂等性与安全性。
