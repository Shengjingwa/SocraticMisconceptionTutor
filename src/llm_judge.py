...
你必须返回 JSON 格式，包含 socratic_degree (整数1-5), teaching_effectiveness (整数1-5) 和 reasoning (字符串)。"""
    ...
    try:
        ...
        print(f"评估出错: {e}")
        return EvaluationOutput(socratic_degree=1, teaching_effectiveness=1, reasoning=f"评估失败: {e}")
...
def main():
    ...
    with open(turn_logs_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            sessions[sid].append({
                "role": "学生",
                "content": turn.get("student_input", "")
            })
            sessions[sid].append({
                "role": "助教",
                "content": turn.get("final_reply", "")
            })
    ...
    for sid, messages in sessions.items():
        ...
        print(f"  -> 苏格拉底度: {eval_output.socratic_degree}, 有效性: {eval_output.teaching_effectiveness}")
