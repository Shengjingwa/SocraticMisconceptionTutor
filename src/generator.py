...
def generate_reply(user_input: str, decision: RouteDecision, memory: SessionMemory, messages: list = None) -> Dict[str, Any]:
    ...
    if decision.state == "S4":
        ...
        if memory.recent_states.count("S5") >= 3:
            fallback_strategy += "\n\n【深度认知冲突与破而后立策略】\n检测到学生处于连续卡壳状态，要求你强制采用“归谬法（Reductio ad absurdum）”或“极端情境法”，顺应学生的错误逻辑推导出一个明显荒谬的后果，以此制造认知冲突。在成功制造冲突后，必须紧接着进行“破而后立”：用通俗的语言指出学生混淆的底层概念（例如：区分“能量消耗”与“电荷守恒”），并提供一个新的、正确的物理框架起点，引导学生重新开始推导。"
        ...
        fallback_strategy += "\n\n【降级干预：事实兜底与重构 (Fact-Grounding & Rebuild)】\n" \
                             "检测到学生陷入极度顽固的认知僵局（P1死锁）。**立即停止所有的抽象类比和归谬反问！**\n" \
                             "为了绝对不触碰泄题红线，你必须采取“先陈述事实，再提供脚手架”的终极策略：\n" \
                             "1. 直接抛出一个无可辩驳的、真实的【物理实验现象或硬核事实】。这个事实必须与学生的错误认知形成极强的反差。\n" \
                             "2. 在陈述完事实后，**严禁直接讲授最终原理解释**！相反，你应该提供一个全新的、正确的分析视角作为“脚手架”。\n" \
                             "3. 例如：‘真实的物理实验结果是 [陈述打脸现象]。既然结果如此，那我们之前的推理肯定是某个基础概念混淆了。你觉得是不是把 A 和 B 搞混了？’"
...
7. 当学生表现出困惑或多次卡壳时，绝不要重复相同的反问，必须提供一个具体的生活类比（如水流、跑步、木块等）或将问题拆解为更小的分步提问。若学生对当前类比产生误解或排斥，绝对不要去解释或为该类比辩护，必须大方承认其局限性并强制将视角拉回目标物理情境本身或极端条件。
8. 绝对不要在回复中包含任何内部思考过程、策略说明或动作提示（如括号内的心理活动）。如果需要思考，请将思考过程写在 <think>...</think> 标签内。在 </think> 之后，你只能提出一个引导性的问题或类比，绝对不能把思考标签里的结论直接写出来。
9. 每次回复只能提出一个清晰的问题，严禁自问自答，严禁同时抛出多个维度的变量（如同时混杂重量、形状和体积）。
10. **防重复与防抽象策略**：面对顽固学生，**绝对不要重复已经失败的类比**。如果学生坚持错误，必须切换到完全不同的、日常生活中极其直观的物理经验（如骑自行车、提重物等），严禁使用复杂的数学抽象（如密度计算、排开水的精确体积）来试图说服学生。
11. **防早退机制**：如果学生即将理解（处于认知冲突中或刚开始动摇），**绝对不要提前结束对话**或说“下次再说”、“今天先到这里”。你必须坚持引导，直到学生得出结论或遭遇无法逾越的僵局（P1死锁）。
