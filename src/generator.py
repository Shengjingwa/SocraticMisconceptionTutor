...
def generate_reply(user_input: str, decision: RouteDecision, memory: SessionMemory, messages: list = None) -> Dict[str, Any]:
    ...
    if decision.state == "S4":
        ...
        fallback_strategy += "\n\n【降级干预：事实兜底 (Fact-Grounding)】\n" \
                             "检测到学生陷入极度顽固的认知僵局（P1死锁）。**立即停止所有的原理解释、抽象类比和归谬反问！**\n" \
                             "为了绝对不触碰泄题红线，你必须采取“只给现象，不给原理”的终极策略：\n" \
                             "1. 直接抛出一个无可辩驳的、真实的【物理实验现象或硬核事实】。这个事实必须与学生的错误认知形成极强的反差和认知冲突。\n" \
                             "2. **严禁提供任何原理解释（0%的保姆级讲授）**！你只能像一个冷酷的实验观察者一样陈述客观结果。\n" \
                             "3. 抛出事实后，直接用一句极其简短的挑战性反问收尾，把解释的压力全部推回给学生。例如：‘但真实的物理实验结果是 [陈述打脸现象]。如果你的想法是对的，那怎么解释这个真实的实验现象？’"
