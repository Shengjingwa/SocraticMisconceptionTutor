...
def _safe_question_template(user_input: str, decision: RouteDecision, memory: SessionMemory) -> str:
    ...
    if decision.state == "S6":
        if tag == "M-ELE-001":
            return "你刚才的表述已经很接近了。请你用自己的话把关键机制讲完整：电流在电路里为什么不会被灯泡“用掉变少”？你可以从“电荷数量”和“能量变化”分别说一句。"
        if tag == "M-ELE-002":
            return "你刚才的推理已经很接近了。请你用自己的话解释：为什么电路需要形成一个闭合回路，电荷才能持续运动？如果只有一根线，会发生什么现象？"
        if tag == "M-BUO-001":
            return "你刚才的想法很接近正确解释了。请你用自己的话总结：物体是“沉还是浮”主要取决于什么？为什么“更重”不等于“更容易沉”？"
        if tag == "M-BUO-002":
            return "你刚才的推理已经很接近了。请你用自己的话总结：同一物体完全没入后继续下沉，浮力会怎么变化？你的理由是什么？"
        return "你已经把关键点抓到了。请你用自己的话把完整的因果链讲一遍（不要只说结论），我会只问一个地方：你这一步依据的是什么现象？"
    if decision.state == "S7":
        if tag == "M-ELE-001":
            return "我们只说一个可重复的实验现象：把两个完全相同的灯泡串联，肉眼观察它们亮度几乎一样；把电流表分别串在灯泡前、两灯之间、灯泡后，三个位置读数一致。你觉得如果电流真的在第一个灯泡里“被用掉变少”，这组读数为什么还能一致？"
        if tag == "M-ELE-002":
            return "我们只说一个可重复的现象：把电池、灯泡、电线搭成“闭合回路”时灯泡能持续点亮；如果只用一根线碰一下，往往只能闪一下或根本不亮。你觉得“持续点亮”这一点，和“电荷有没有回去的路”有什么关系？"
        if tag == "M-BUO-001":
            return "我们只说一个可重复的现象：一大块木头通常比一小颗石子重，但木头能浮、石子会沉。你觉得这说明“沉浮”决定于什么，而不只是“重不重”？"
        if tag == "M-BUO-002":
            return "我们只说一个可重复的现象：同一个物体完全没入水里后继续下沉，它排开水的体积基本不变；但浮力并不会因此不断变大到把它顶上来。你觉得这里最关键的矛盾点是什么？"
        return "我们先只承认一个实验现象（不解释原因）：在这个现象下，你原先的解释会推出什么可观察的结果？它和现象一致吗？"
